const abi = require('../../contract/artifacts/contracts/BridgeFabric.sol/BridgeFabric.json')
...
export const getNodePrice = async() => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    nodePrices.push({
      node: nodes[i],
      price: await contract.getNodePrice(nodes[i])
    })
  }
...
export const countBuy = async(host, actor, count) => {
  ...
  try {
    const transaction = await contract.countBuy(host, actor, count)
...
export const countCurrent = async(host, actor) => {
  const provider = await providerInit()
  const contract = new ethers.Contract(contractAddress, abi, provider)
  const count = await contract.countCurrent(host, actor)
  return count
}
